...
架空のサービスではありますが、一般的なECサイトのマーケットプレイスのように、売り手と買い手が存在し、売り手は商品の登録や変更、削除、買い手は商品の検索、決済、購入履歴の確認といった操作を行えるようにしています。ホスティングもしていますので、実際に[eFarm](https://rkrk.pythonanywhere.com/)[^1]を確認することができます。
...
- 商品一覧から特定の商品を検索することもできます。検索フローは次のようになっています。
  ...
  3. forループでwordsのリスト内にある単語を一つずつ処理する。検索したユーザーがBuyerまたはユーザー未登録であればProductオブジェクト全件に対して、Sellerであれば自身の登録したProductオブジェクトに対して検索を行う。
...
- 商品一覧から商品詳細に遷移すると、商品名や商品説明、個数や価格が表示されます。Productモデルのpriceフィールドは税抜価格となっているため、商品詳細画面で価格を表示する際には、同モデル内で定義したprice_with_tax関数から税込価格を表示するようにしています。priceフィールドを税抜価格にし、税込価格は関数で計算するようにしたのは、消費税率が変更されたときの対応を簡単にするためです（現在はtax=0.1で設定）。
- 商品詳細から商品を登録したSellerの詳細画面へ遷移することもできます。また、ユーザーがBuyerの場合は商品の購入個数を選択してカートに入れることができます。ユーザー未登録の場合はカートのフォームは表示されません。
...
- カートに商品を登録するフローは次のようになっています。
  ...
  4. Cartに関連付けられているCartItemオブジェクトに同じ商品が既にある場合は購入個数と金額を更新する。同じ商品がない場合は新たにCartItemオブジェクトを作成する。
...
- 決済のフローは次のようになっています。
  ...
  4. StripeAPIのCheckoutセッションを作成する。Checkoutセッションのline_items属性に、ディクショナリにしたカート内の各商品をリストに入れて渡す。また、customer属性にはpayment_idを設定している。
...
- StripeのWebhookエンドポイントを登録しているため、決済完了画面の裏では決済履歴の作成が自動で実行されています。ここでPayment,PaymentItemオブジェクトが作成されます。決済履歴を作成するフローは次のようになっています。
   ...
   6. 登録された決済履歴はBuyerのマイページにあるPaymentListから確認できる。

## templates
- accounts,products,carts,paymentsの各アプリケーションで使用するテンプレートファイルを格納するディレクトリです。
- ディレクトリ直下に配置したbase.htmlを親テンプレートとし、各アプリケーションのテンプレートファイルに共通要素であるヘッダーやフッターを継承しています。
- 各アプリケーションのテンプレートファイルは子テンプレートとして、親テンプレートであるbase.htmlのcontentブロックをオーバーライドすることで、テンプレートファイルごとに異なる要素を表示しています。
- base.htmlやユーザープロフィール、商品詳細、決済履歴のテンプレートファイルなど一部のファイルでは、ユーザー情報等に基づきDjangoテンプレート言語の変数やタグ、フィルタで表示内容を変更しています。

## tests
- accounts,products,carts,paymentsの各アプリケーションのテストをまとめたディレクトリです。
- 各アプリケーションのviewに対して、viewで使われるオブジェクトの登録状況に応じて、意図したとおりにそれぞれのviewが機能するかをテストしています。
- 例えば、accountsアプリケーションのviewについて、CustomUserのis_buyerフラグがTrueのユーザーがBuyerオブジェクトの作成や更新を正しくできるか、反対にSellerオブジェクトの作成や更新をしようとすると拒否されるかを、Httpレスポンスのステータスコードや使用されるテンプレートファイル、レスポンスに含まれる文言などでテストしています。
- testsディレクトリにある画像ファイルは、productsアプリケーションのviewで画像ファイルのアップロードをテストする際に使います。テストコードで作成したオブジェクトはテストが終了すると自動で削除されますが、media/uploads/producut_imgsにテストでアップロードした画像ファイルは自動で削除されないので、テストの最後にアップロード先のパスからファイルを削除しています。
